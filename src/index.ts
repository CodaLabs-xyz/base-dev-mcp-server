...
interface ScrapedData {
  ...
  content: Array<ContentItem | PageData>;
}
...
class BaseMiniKitMCPServer {
  ...
  private documentationCache: Map<string, ScrapedData> = new Map();
...
export function MiniKitContextProvider({ children }) {
  ...
  private async getBaseContracts(args: any) {
    ...
    for (const [net, netContracts] of Object.entries(networkData)) {
      result[net] = contractType === 'all' ? netContracts : { [contractType]: (netContracts as any)[contractType] };
    }
  ...
  private async getBaseRPCInfo(args: any) {
  ...
  private async getOnchainKitComponents(args: any) {
...
function TokenSwap() {
  ...
  private async getBaseEcosystem(args: any) {
  ...
  private async generateBaseContract(args: any) {
...
contract ${name} {
      ...
      content: [
        {
          type: 'text',
          text: JSON.stringify({
            contract: (contracts as any)[contractType],
            gasOptimizations: [
  ...
  private async getBaseDeploymentGuide(args: any) {
...
const contractAddress = await sdk.deployer.deployContract({
  ...
  private async getBaseBridgeInfo(args: any) {
    ...
    const result = bridgeType === 'all' ? bridges : { [bridgeType]: (bridges as any)[bridgeType] };
  ...
  private async scrapeBaseDocs(args: any) {
    ...
    try {
      ...
      if (depth === 'deep' && section && section !== 'all') {
        const sectionLinks = scrapedData.navigation.filter(link => 
          link.section === section || link.href.includes(section)
        );
        ...
        for (const link of sectionLinks.slice(0, 5)) { // Limit to 5 additional pages
          try {
            ...
            console.error(`Error scraping ${link.href}:`, (error as Error).message);
          }
        }
      }
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to scrape Base documentation: ${(error as Error).message}`
      );
    }
  }

  private async searchBaseDocs(args: any) {
    ...
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to search Base documentation: ${(error as Error).message}`
      );
    }
  }

  private categorizeLink(href: string): string {
    if (href.includes('get-started')) return 'get-started';
    if (href.includes('base-chain')) return 'base-chain';
    if (href.includes('base-account')) return 'base-account';
    if (href.includes('base-app')) return 'base-app';
    if (href.includes('onchainkit')) return 'onchainkit';
    if (href.includes('cookbook')) return 'cookbook';
    if (href.includes('learn')) return 'learn';
    return 'other';
  }
